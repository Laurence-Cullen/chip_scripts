...
def main(fid):
    ...
    #Plot attempt
    x, y, z = fid_to_coords(fid, 5, collect_ordr_dict)
    fig = plt.figure()
    fig = plt.figure(figsize=(10,10), facecolor='white', edgecolor='k')
    ax1 = fig.add_subplot(111, aspect=1, axisbg='r')
    fig.subplots_adjust(left=0.03,right=0.97,bottom=0.03,top=0.97)
    if fid.endswith('spots'):
        plt.scatter(x, y, c=z, s=41, alpha=1, marker='s', cmap='PuOr')
    else:
        print type(z)
        z = (-1*z)
        #plt.scatter(x, y, c=z, s=41, alpha=1, marker='s', cmap='Oranges')
        #plt.scatter(x, y, c=z, s=41, alpha=1, marker='s', cmap='PuOr')
        plt.scatter(x, y, c=z, s=41, alpha=1, marker='s', cmap='Greys')
